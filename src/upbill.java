...
class upbill extends JFrame implements ActionListener
{
    long mn;
    Statement s;
    Connection c;
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(e.getSource()==b1)
        {
            ...
            try
            {
                ...
                ss=t5.getText();
                si=t7.getText();
                sb=String.valueOf(((Integer.parseInt(sl)-Integer.parseInt(t2.getText()))*1)+((Integer.parseInt(ss)-Integer.parseInt(t4.getText()))*2)+((Integer.parseInt(si)-Integer.parseInt(t6.getText()))*5));
                s.executeUpdate("update MobileBill set Local='"+sl+"',STD='"+ss+"',ISD='"+si+"',Bill='"+sb+"' where Numbers='"+sm+"'");
                JOptionPane.showMessageDialog(this,"Bill is Updated","DONE!!! ",JOptionPane.INFORMATION_MESSAGE);
                upbill u2=new upbill();
                u2.setTitle("TO Update Bill Detail.");
                u2.setSize(800,600);
                u2.setVisible(true);
